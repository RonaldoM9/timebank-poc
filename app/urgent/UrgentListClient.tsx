  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a]">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* Filtres */}
        <div className="bg-[#111111] border border-[#262626] rounded-2xl p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
            <input
              ...
              className="w-full bg-[#0a0a0a] border border-[#262626] rounded-xl px-4 py-2.5 text-[#f5f5f5] placeholder:text-[#5c5c5c] focus:outline-none focus:border-[#f59e0b] transition-colors text-sm"
            />
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full bg-[#0a0a0a] border border-[#262626] rounded-xl px-4 py-2.5 text-[#f5f5f5] focus:outline-none focus:border-[#f59e0b] transition-colors text-sm appearance-none"
            >
              {CATEGORIES.map((cat) => (
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-[#262626] bg-[#0a0a0a] text-[#f59e0b] focus:ring-[#f59e0b]"
                />
                <span className="text-[#a3a3a3] text-sm">En ligne</span>
              </label>
            </div>
          </div>
        ...
        {loading && (
          <div className="text-center py-8">
            <p className="text-[#5c5c5c] text-sm">Recherche en cours…</p>
          </div>
        )}

        {/* Results */}
        {!loading && requests.length === 0 && (
          <div className="text-center py-16 bg-[#111111] border border-[#262626] rounded-2xl">
            <Zap className="w-12 h-12 text-[#5c5c5c] mx-auto mb-4" />
            <h2 className="text-lg font-semibold text-[#a3a3a3] mb-1">Aucune demande pour le moment</h2>
          ...
          <div className="space-y-4">
            {requests.map((req) => (
              <Link
                key={req.id}
                href={`/urgent/${req.id}`}
                className="block bg-[#111111] border border-[#262626] rounded-2xl p-5 hover:border-[#f59e0b]/30 transition-all group"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-lg font-anton tracking-wide text-[#f5f5f5] group-hover:text-[#f59e0b] transition-colors mb-1">
                      {req.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-4 text-xs text-[#5c5c5c]">
                      ...
                      <div className="flex items-center gap-1">
                        <Users className="w-3.5 h-3.5 text-[#5c5c5c]" />
                        {req.requester.name}
                      </div>
                    </div>
                  </div>
