...
import NFCCompleteClient from "./NFCCompleteClient";
import crypto from "crypto";

export default async function NFCCompletePage({
  params,
}: {
  ...
  if (!session?.user?.email) {
    redirect(`/auth/signin?callbackUrl=/complete/nfc/${token}`);
  }
  ...
  if (!user) redirect(`/auth/signin?callbackUrl=/complete/nfc/${token}`);

  // Vérifier le token côté serveur (method = nfc uniquement)
  ...
  if (!booking) {
    error = "Lien NFC invalide ou inexistant";
    ...
    error = "Le provider ne peut pas valider son propre NFC";
