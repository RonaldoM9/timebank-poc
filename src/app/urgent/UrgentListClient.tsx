...
export default function UrgentListClient({ initialRequests }: { initialRequests: UrgentRequest[] }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 bg-tb-bg">
      <ConnectedHeader />

      <main className="max-w-5xl mx-auto px-4 py-8 animate-fade-in-up">
        {/* Hero section */}
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 via-[#d97706] to-[#b45309] p-6 mb-8">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-bangers text-yellow-200/80 text-xs tracking-wider">
                ~ besoin urgent local ~
              </span>
            </div>
        ...
        {/* Filtres */}
        <div className="bg-tb-surface border border-tb-border rounded-2xl p-5 mb-6 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-500/5">
          <div className="flex items-center gap-2 mb-4">
            <Search className="w-4 h-4 text-tb-text-muted" />
            <span className="text-sm font-semibold text-tb-text-muted uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
            <input
              ...
              className="w-full bg-tb-surface border border-tb-border rounded-xl px-4 py-2.5 text-tb-text-primary placeholder:text-tb-text-muted focus:outline-none focus:border-[#f59e0b] transition-colors text-sm"
            />
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="w-full bg-tb-surface border border-tb-border rounded-xl px-4 py-2.5 text-tb-text-primary focus:outline-none focus:border-[#f59e0b] transition-colors text-sm appearance-none"
            >
              {CATEGORIES.map((cat) => (
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 rounded border-tb-border bg-tb-surface text-[#f59e0b] focus:ring-[#f59e0b]"
                />
                <span className="text-tb-text-secondary text-sm">En ligne</span>
              </label>
            </div>
          </div>
        ...
        {loading && (
          <div className="text-center py-8">
            <p className="text-tb-text-muted text-sm">Recherche en cours…</p>
          </div>
        )}

        {/* Results */}
        {!loading && requests.length === 0 && (
          <div className="text-center py-16 bg-tb-surface border border-tb-border rounded-2xl transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg">
            <Zap className="w-12 h-12 text-tb-text-muted mx-auto mb-4" />
            <h2 className="text-lg font-semibold text-tb-text-secondary mb-1">Aucune demande pour le moment</h2>
            <p className="text-tb-text-muted text-sm mb-4">
          ...
          <div className="space-y-4">
            {requests.map((req) => (
              <Link
                key={req.id}
                href={`/urgent/${req.id}`}
                className="block bg-tb-surface border border-tb-border rounded-2xl p-5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-amber-500/5 hover:border-[#f59e0b]/30 group"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-lg font-anton tracking-wide text-tb-text-primary group-hover:text-[#f59e0b] transition-colors mb-1">
                      {req.title}
                    </h3>
                    <p className="text-tb-text-secondary text-sm line-clamp-2 mb-2">
                      {req.description}
                    </p>
                    <div className="flex items-center gap-4 text-xs text-tb-text-muted">
                      ...
                      <div className="flex items-center gap-1">
                        <Users className="w-3.5 h-3.5 text-tb-text-muted" />
                        {req.requester.name}
                      </div>
                    </div>
                  </div>
