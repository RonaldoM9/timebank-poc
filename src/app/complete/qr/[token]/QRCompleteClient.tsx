...
import { ArrowLeft, CheckCircle, XCircle, Clock, User, Shield, Hourglass, AlertTriangle, QrCode } from "lucide-react";
import { validateQRProof } from "@/app/services/qr-actions";
...
export default function QRCompleteClient({
  token,
  booking,
  error,
  userId,
  ...
  async function handleConfirm() {
    ...
    const result = await validateQRProof(token);
    if ("error" in result) {
      ...
      <main className="max-w-lg mx-auto px-4 py-8">
        <div className="bg-[#ffffff] border border-[#e5e5e5] rounded-2xl p-6 sm:p-8">
          ...
          <p className="text-[#9ca3af] text-xs text-center mt-4">
            En confirmant, vous attestez que la mission a été réalisée conformément à la réservation.
          </p>
        </div>
      </main>
    </div>
