...
import ProgramImpactClient from "./ProgramImpactClient"

export default async function ProgramImpactPage({
  params,
}: {
  ...
  const canViewStats = membership && ["OWNER", "ADMIN", "FACILITATOR"].includes(membership.role)
  const canViewFull = membership && ["OWNER", "ADMIN"].includes(membership.role)
