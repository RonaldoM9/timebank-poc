...
export async function createService(formData: FormData) {
  ...
  if (isSolidarityMission && !solidarityCategory) {
    return { errors: { solidarityCategory: ["La catégorie solidaire est requise"] } };
  }
  if (isSolidarityMission && !solidarityReason) {
    return { errors: { solidarityReason: ["La justification solidaire est requise"] } };
  }

  const solidarityData = isSolidarityMission
    ? {
        isSolidarityMission: true,
        solidarityStatus: "SELF_DECLARED",
        solidarityCategory: solidarityCategory ?? null,
        solidarityReason: solidarityReason ?? null,
      }
    : {
        isSolidarityMission: false,
        solidarityStatus: "CLASSIC",
        solidarityCategory: null,
        solidarityReason: null,
      };
