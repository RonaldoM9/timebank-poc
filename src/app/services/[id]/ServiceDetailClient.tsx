...
import { Clock, ArrowLeft, Calendar, User, Shield, MapPin, HeartHandshake } from "lucide-react";
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="bg-tb-surface border border-tb-border rounded-2xl p-6 sm:p-8">
          ...
          {/* ─── Lot 18 — Mission Solidaire Info + Pot Commun ─────────── */}
          {service.isSolidarityMission && (
            <div className="bg-teal-500/5 border border-teal-500/20 rounded-xl p-4 mb-6">
              <h2 className="text-xs font-semibold text-teal-400 mb-2 uppercase tracking-wider flex items-center gap-1.5">
                <HeartHandshake className="w-3.5 h-3.5" />
                Mission solidaire
              </h2>
              {service.solidarityCategory && (
                <p className="text-sm text-tb-text-primary mb-1">
                  <span className="text-tb-text-secondary">Catégorie :</span>{" "}
                  {SOLIDARITY_CATEGORY_LABELS[service.solidarityCategory] || service.solidarityCategory}
                </p>
              )}
              {service.solidarityReason && (
                <p className="text-sm text-tb-text-secondary mb-1">
                  <span className="text-tb-text-primary">Justification :</span> {service.solidarityReason}
                </p>
              )}
          ...
          {/* Pot commun — for solidarity missions */}
          {session && !isOwner && service.isSolidarityMission && (
            <div className="bg-purple-500/5 border border-purple-500/20 rounded-xl p-4 mb-6">
              <p className="text-sm text-tb-text-secondary">
                <span className="text-tb-text-primary font-medium">💰 Pot commun disponible</span>
                <br />
                Cette mission solidaire est éligible à une demande d&apos;aide du pot commun TimeHeroes
                après réservation.
              </p>
            </div>
          )}
