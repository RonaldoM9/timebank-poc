  ...
  return (
    <div className="bg-tb-surface border border-tb-border rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-tb-text-primary flex items-center gap-2">
          🎯 Parcours de découverte
        </h3>
        {allDone ? (
          <span className="text-xs text-tb-accent font-medium">✅ Félicitations !</span>
        ) : (
          <span className="text-xs text-tb-text-secondary">
            ...
            <div key={i} className="flex items-center gap-3 py-1.5 group">
              ...
              >
                {done ? (
                  ...
                  <Circle className="w-4 h-4 text-tb-text-muted group-hover:text-tb-text-secondary transition-colors" />
                )}
              </button>
              <Link
                href={step.href}
                className={`text-sm transition-colors ${
                    ...
                    ? "text-tb-text-muted line-through"
